...
class POSController extends POSControllerState with 
  ...
  void _setupSocketListenersDetailed() {
    ...
    socket.onPrintRequest((data) async {
      if (!isMainPrinterTerminal.value) {
        Get.snackbar("Print Request Received", "This terminal is NOT set as Main Printer. Skipping print.", 
          backgroundColor: Colors.orange.withOpacity(0.9), colorText: Colors.white, duration: const Duration(seconds: 2));
        return;
      }
  ...
  Future<void> printBillAndExit() async {
    ...
    try {
      ...
      if (editingOrderId.value != null) {
        processedPrintIds["${editingOrderId.value}_hisob cheki"] = DateTime.now();
      }
